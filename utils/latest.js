...
const executeQuery = async () => {
    ...
    const cdcQuery1 = 'with \n' +
        ...
        ' 		 (select sum(val)/10000000 from tot where type1 = \'ODD\' and baldt = (current_date -n)      and n=1) odd_amt,\n' +
        ' 		 (select sum(val)/10000000 from tot where type1 = \'ODD\' and baldt = (current_date -n)      and n=2) podd_amt,\n' +
        ' 		 (select sum(val)/10000000 from tot where "Typ" in (\'D\',\'O\') and baldt = (current_date -n) and n=1) tot_dep,\n' +
        ' 		 (select sum(val)/10000000 from tot where "Typ" in (\'D\',\'O\') and baldt = (current_date -n) and n=2) pdt_tot_dep,\n' +
    ...
    // console.log('starting flag query')
    // try {
    //     const resultFlag = await pool.query(flagQuery)
    //     var flagToCron = resultFlag.rows[0].ready2go
    //     console.log(flagToCron)
    // } catch (error) {
    //     console.log(error)
    // }
    // console.log('flag query completed')
    // if (flagToCron) {
